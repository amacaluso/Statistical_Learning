...
evaluation_model( target_variable = Y_test, prediction = lm_pred_test, MODEL = 'Multiple Linear Regression' )
...
Leap_forward = ggplot( data = df, aes( x = n_var, y = RMSE)) + geom_line(col = "blue") + 
               ...
               geom_segment( aes( x = n_var, y=RMSE-sdev, xend=n_var, yend = RMSE+sdev ))

Leap_forward = ggplotly( Leap_forward )

save_plot( Leap_forward, type = 'REGRESSION' )
...
anova(linear_reg, gam.3.fit, test = "Chisq")
